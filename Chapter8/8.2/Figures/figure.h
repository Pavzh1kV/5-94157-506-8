...
public :
    ...
    virtual void putPoint(int x, int y)
    {
        putPoint(Point(x, y));
    }
    virtual void putPoint(const Point & pt)
    {
        putPoint(pt.getX(), pt.getY());
    }

    // Функции стирания точки:
    // один из двух вариантов должен быть переопределен
    virtual void clearPoint(int x, int y)
    {
        clearPoint(Point(x, y));
    }
    virtual void clearPoint(const Point & pt)
    {
        clearPoint(pt.getX(), pt.getY());
    }
...
public :
    ...
    void draw(Canvas & c, int x, int y) const
    {
        baseFigure1->draw(c, x, y);
        baseFigure2->draw(c, x, y);
    }
};
